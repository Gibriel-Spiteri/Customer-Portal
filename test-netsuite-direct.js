  ...
  try {
    ...
    const growthData = monthlyData.map((month, index) => {
      ...
      if (previousMonth && previousMonth.new_customers > 0) {
        ...
        growthRate = (
          ((month.new_customers - previousMonth.new_customers) /
    ...
    const totalCustomers = monthlyData.reduce(
      (sum, month) => sum + month.new_customers,
      0,
    );
    const avgPerMonth =
      monthlyData.length > 0
        ? (totalCustomers / monthlyData.length).toFixed(1)
        : 0;
    const lastMonth = monthlyData[0]?.new_customers || 0;
    const lastThreeMonths = monthlyData
      .slice(0, 3)
      .reduce((sum, month) => sum + month.new_customers, 0);
    const lastSixMonths = monthlyData
      .slice(0, 6)
      .reduce((sum, month) => sum + month.new_customers, 0);
    const lastYear = monthlyData
      .slice(0, 12)
      .reduce((sum, month) => sum + month.new_customers, 0);
    ...
    growthData.slice(0, 12).forEach((month) => {
      ...
      const growthAmountStr =
        month.growth_amount !== null ? String(month.growth_amount) : "N/A";

      console.log(
        `${monthStr} | ${newCustomersStr} | ${growthRateStr} | ${growthAmountStr}`,
      );
    });
    ...
    if (error.message.includes("configured")) {
      ...
      console.error(
        "- NETSUITE_PRIVATE_KEY (or netsuite_private_key.pem file)",
      );
    }
  }
}
