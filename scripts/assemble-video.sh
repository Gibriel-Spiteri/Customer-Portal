...
TRIM=1.5    # seconds trimmed from clip start
...
drawtext=fontfile=$FONT:text='$3':fontcolor=0xBBD0F0:fontsize=36:x=(w-text_w)/2:y=205:alpha='if(lt(t,$sw-0.4),1,if(lt(t,$sw),($sw-t)/0.4,0))',\
drawtext=fontfile=$FONT:text='$4':fontcolor=0xBBD0F0:fontsize=36:x=(w-text_w)/2:y=205:alpha='if(lt(t,$sw),0,if(lt(t,$sw+0.4),(t-$sw)/0.4,1))',\
