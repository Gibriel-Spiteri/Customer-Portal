...
export interface IStorage {
  ...
  getUserByNetsuiteId(netsuiteCustomerId: string): Promise<User | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByNetsuiteId(netsuiteCustomerId: string): Promise<User | undefined> {
    const [user] = await db.select().from(users).where(eq(users.netsuiteCustomerId, netsuiteCustomerId));
    return user || undefined;
  }
